    ...
    public void onClick(View v) {
        if(v == startRecordTxt){
            ...
            AliyunSnapVideoParam recordParam = new AliyunSnapVideoParam.Builder()
                    //录制 设置录制分辨率，目前支持360p，480p，540p，720p
                    .setResolutionMode(resolutionMode)
                    //录制 设置视频比例，目前支持1:1,3:4,9:16
                    .setRatioMode(ratioMode)
                    //录制 设置录制模式，目前支持按录，点录和混合模式
                    .setRecordMode(AliyunSnapVideoParam.RECORD_MODE_AUTO)
                    //录制 设置滤镜地址列表,具体滤镜接口接收的是一个滤镜数组
                    ...
                    //设置关键帧间隔
                    .setGop(gop)
                    //录制设置视频码率，如果不设置则使用视频质量videoQulity参数计算出码率
                    //.setVideoBitrate(2000)
                    //裁剪 设置过滤的视频最小长度 单位毫秒
                    //.setMinVideoDuration(minVideoDuration)
                    //裁剪 设置过滤的视频最大长度 单位毫秒
